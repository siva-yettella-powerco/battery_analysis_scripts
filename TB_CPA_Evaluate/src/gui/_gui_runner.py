"""
_gui_runner.py  —  Subprocess shim for evaluate_gui.py
========================================================
Reads a JSON config file passed as argv[1], calls run_evaluate(**kwargs).
...
# Package root is three levels up from src/gui/
...
if __name__ == "__main__":
    ...
    print(f"[GUI Runner] Starting evaluation: {kwargs.get('name', '?')}", flush=True)
    print(f"[GUI Runner] Base path: {kwargs.get('base_path', '?')}", flush=True)

    # Remove GUI-only keys not accepted by run_evaluate
